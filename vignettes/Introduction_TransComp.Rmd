...
Here the function *loadAndCleanData* is a function that standardizes a dataset to include the appropriate column format for later analysis, using IDs derived from ensembl.
...
To visualize these results, you may use *PlotCanTrans* function. *PlotCanTrans* plots the proportion of transcripts found for the genomic coordinates in your dataset as canonical, calculated as in *CanTrans*. This relative proportion is visualized using two different colors for every different gene in your dataset. To read more about *PlotCanTrans* function, see:
...
We can also identify possible transcripts for a specified gene and sample, and then do this for every gene and sample in the given dataset using *TransComp*. This will give us all the transcripts for this gene that have every exon sequenced in the specified sample. We can also find overlapping transcripts between two datasets using *CompTransc*.

```{r, }
transComp::TransComp(formattedData)
```

The results from TransComp can be plotted using *PlotTransComp* to display the count of transcripts for each gene:

```{r}
transComp::plotTransComp(formattedData, "Example")
```
