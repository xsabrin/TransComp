...
plotTransComp <- function(transData, datasetName=NA) {
  ...
  # plot data
  plot <- ggplot2::ggplot(plotData, aes(x = ensembl_gene_id,
                                        fill = ensembl_transcript_id)) + geom_bar() +
    labs(title = plotTitle, x = "Ensembl Gene ID", y = "Possible Transcripts (count)",
         fill = "ensembl_transcript_id")
